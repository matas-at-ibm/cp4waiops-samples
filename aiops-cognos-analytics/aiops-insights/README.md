...
AIOps Insights CA dashboard is a rendition of the [AIOps Insights](https://www.ibm.com/docs/en/cloud-paks/cloud-pak-aiops/latest?topic=insights-aiops-ui) capability re-imagined with Db2 and CA (Cognos Analytics). The CA dashboard offers visualizations of much the same AIOps processing metrics as the in-product dashboard. It does have a limitation of not presenting the runbook-related metrics but it makes up for it in the flexibility to customize the dashboard at will with schema modifications and further visualization extensions via CA visualization library. 
...
Then the creation scripts must be sequentially executed as detailed below (instructions assume that db2 directory has been copied over as the db2 directory on the Db2 hosting machine ):
```
...
*optional\**: `aiops_insights.sql` is not reliant on `reporter_aiops_noise_reduction.sql` and can be ignored.


##### Addition of AIOps Insights Schema
...
##### Db2 modifications
AIOps Insights schema makes use of Db2 <b>Administrative Task Scheduler</b> to optimize data access by refreshing the metrics views every 60s. To receive updated metrics for the charts, ensure that Db2 Administrative Task Scheduler is enabled by following the instructions [here](https://www.ibm.com/docs/en/db2/11.5.x?topic=scheduler-setting-up-administrative-task).


#### Db2 AIOps schema cleanup
> IMPORTANT: If the intention is to also remove the Reporter schema, please run the below file first
...
![](./images/alerts-policy-triggers-and-condition-sets.png)
...
##### 13. Relink the Insights Data Module. Access the data module under Content > Team content > AIOps Insights > Insight Data Module. Then navigate to the Sources pane, select the broken source, choose "Relink" from the context menu and select the db2 data server created in the [Set up Db2 Data Server in Cognos Analytics](#set-up-db2-data-server-in-cognos-analytics) section.
...
The visualizations in the Dashboard are configured to refresh every 60 seconds. To provide faster load times, the expensive views used to populate the charts are maintained as DB2 materialized query tables and are refreshed every 60 seconds. The latest refresh time for the materialized query tables can be seen in the widget with text `Last refresh at: x`  at the bottom of the dashboard
...
- <b>Resource group scope</b>: will only include the alerts that occurred on the selected resource group and the incident they belong to.

While the options for filtering by time windows and incident priorities are static, the resource group scopes are estate-dependant.
...
To define a new resource group scope, an SQL insert statement will need to be issued against the `RESOURCE_GROUP_SCOPES` table. Below is the template for the insert statement:
```
 INSERT INTO RESOURCE_GROUP_SCOPES VALUES (
    ...
    <ID_USED_FOR_DIFFERENTIATING_BETWEEN_THE_GROUPS> 
 )@
```
...
```
The `RESOURCE_GROUP_TAGS.TAG` will be populated with:
- `Service to service to database to hypervisor`
- `Service to service to users-api.myapp.example.com to database`
...
Also multiple rows can be used to define a single scope. This can be achieved by keeping the value of `RESOURCE_GROUP_SCOPES.SCOPEID` identical for rows that define a single scope.
...
If any issues arise, troubleshooting should be completed in the following order:
